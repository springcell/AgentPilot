...
def _is_valid_reply(text: str, task_text: str) -> bool:
    """
    合法回复 = 包含 JSON 块 OR 任务完成标记 OR 纯对话回复。
    其余所有情况都视为异常，进入修复循环。
    """
...
_ACTION_TASK_PATTERNS = [
    r"启动|打开|运行|执行|安装|部署|修复|修改|创建|生成|写入|保存|上传|推送|提交|美化|修改",
...
def run_agent(user_task: str, verbose: bool = True) -> str:
    ...
    _invalid_reply_count = 0   # 连续无效回复计数
    ...
    while iteration < MAX_ITERATIONS:
        ...
        if not all(r["success"] for r in results):
            ...
            err_summary = "|".join(
                (r.get("stderr") or "")[:80] for r in failed
            )
...
_REFUSAL_PATTERNS = [
    ...
    r"issue with opening", r"issue with.*file", r"platform.*cannot",
    r"try.*local.*python", r"please.*upload.*file", r"upload.*to.*platform",
]
...
def run_agent(user_task: str, verbose: bool = True) -> str:
    ...
    agent_id_main = "default"
    # 自修复循环状态
    _error_history: list[str] = []   # 历次错误摘要，用于检测重复
    _last_failed_blocks: list[dict] = []  # 上一轮失败的 JSON 块，防止重复 patch
